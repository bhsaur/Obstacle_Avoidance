#!/bin/bash
# Step CR -- 5 heavy-only + 5 flow-only closed-loop runs against Zone B,
# BN seeds, against the CP/CQ-fixed controller (time-based hysteresis,
# hysteresis_min_seconds=0.12, replacing hysteresis_min_frames=3 -- see
# README's Step CQ). Re-run of Step CO's batch, which collided 5/5 in
# both arms under the old frame-count debounce.
#
...
echo "=== $(date -Iseconds) Step CR batch complete ==="
